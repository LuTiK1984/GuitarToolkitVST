...
GuitarToolkit may grow a local music generation module for guitar-focused inspiration. The direction is **ONNX-first**: start with a real local model for symbolic progression tokens, then add a separate small Transformer track for short melody/riff phrases when the dataset, validation layer, and product workflow are ready.
...
Use specialized local models for different musical jobs instead of forcing one model to do everything too early.

The first ONNX model should be a **GRU/LSTM next-token progression model**, not a large Transformer. Its job is narrow: predict the next roman-numeral chord token from style, mode, mood, and previous progression tokens.
...
A second model line should be a **small Transformer melody model**. It should not replace the progression GRU/LSTM at first; it should handle a different task: generating short melodic phrases constrained by the selected meter/bar length.

Why a separate small Transformer later:

- better long-context handling;
- more modern sequence modeling;
- more useful for note/duration patterns than the first progression model;
- better suited for melodic phrases that need local rhythmic shape;
- lets the progression model stay small, stable, and easy to validate.

The melody Transformer should generate phrases, not complete songs.

Recommended first melody task:

```text
Model: MelodyPhraseTransformer
Task: generate a short scale-degree melody phrase inside the selected meter
Input: key + mode + style + mood + meter + bar count + optional chord/progression context
Output: scale-degree + duration tokens for 1-4 bars
```

Example:

```text
Input: E minor, STYLE_METAL, MOOD_DARK, 4/4, 2 bars, i - VI
Output: 1:1/8, b3:1/8, 4:1/8, 5:1/8, b7:1/4, 5:1/4, 4:1/8, b3:1/8
```

Then C# maps the phrase to notes, guitar positions, playback, and later MIDI.
...
Recommended melody Transformer size, later:
...
#### Phase 4: Melody Phrase Transformer

Do not start the whole feature with melodies. Add a separate small Transformer only after progression generation is useful. Do not generate absolute notes first; generate scale degrees and durations, then map them to notes and guitar positions.

The melody model should generate short phrases in the selected meter, for example 1, 2, or 4 bars of `4/4`, `3/4`, `6/8`, or another supported meter. It should not attempt full song composition.
...
- phrase duration fits the selected meter and bar count;
- output can be regenerated against an existing progression context;
- guitar validation can reject or simplify unplayable phrases;
- output can later be converted to MIDI.

#### Phase 5: Model Coordination and Comparison

After both model lines exist, compare and coordinate them instead of replacing one with the other by default.

Recommended constraints:

- keep the token vocabulary symbolic and small;
- keep the GRU/LSTM progression model as the first stable progression baseline;
- use the Transformer primarily for short melody/riff phrases;
- let melody generation optionally condition on generated or user-provided progressions;
- keep model size small enough to ship with the app if licensing allows;
- compare any Transformer progression experiment against the GRU/LSTM baseline before replacing it;
- keep GRU/LSTM progression inference as a fallback if Transformer inference is unavailable.
...
- add a separate small Transformer for short melody/riff phrases after the GRU/LSTM progression baseline is useful;
...
В проект можно добавить локальный модуль музыкальной генерации для гитарного вдохновения. Новое направление - **ONNX-first**: начинаем с настоящей локальной модели для symbolic progression tokens, затем добавляем отдельную линию маленького Transformer для коротких мелодических фраз и риффов, когда будут готовы датасет, validation layer и продуктовый workflow.
...
Лучше использовать специализированные локальные модели под разные музыкальные задачи, а не пытаться слишком рано заставить одну модель делать всё.

Первая ONNX-модель должна быть **GRU/LSTM next-token progression model**, а не большой Transformer. Её задача узкая: предсказывать следующую roman-numeral ступень аккорда по style, mode, mood и предыдущим токенам прогрессии.
...
Вторая модельная линия должна быть **маленьким Transformer для мелодий**. Сначала он не заменяет GRU/LSTM для прогрессий, а решает другую задачу: генерирует короткие мелодические фразы в выбранном размере такта.

Почему отдельный Transformer позже:

- лучше ловит длинный контекст;
- современнее как sequence model;
- лучше подходит для паттернов нот и длительностей, чем первая модель прогрессий;
- лучше работает с локальной ритмической формой мелодической фразы;
- позволяет оставить модель прогрессий маленькой, стабильной и легко валидируемой.

Мелодический Transformer должен генерировать фразы, а не целые песни.

Первая задача для мелодической модели:

```text
Model: MelodyPhraseTransformer
Task: сгенерировать короткую scale-degree melody phrase в выбранном размере
Input: key + mode + style + mood + meter + bar count + optional chord/progression context
Output: scale-degree + duration tokens на 1-4 такта
```
...
Затем C# переводит фразу в ноты, позиции на грифе, проигрывание и позже MIDI.
...
Размер Transformer-модели для мелодий, позже:
...
4. **Melody Phrase Transformer**
   Только после полезного генератора прогрессий. Отдельный маленький Transformer генерирует короткие фразы в выбранном размере такта, например 1, 2 или 4 такта `4/4`, `3/4`, `6/8` или другого поддержанного размера. Мелодии лучше генерировать как scale degrees + durations, а затем переводить в ноты и позиции на грифе. Цель - фраза, а не целая песня.

5. **Model coordination**
   После появления обеих линий моделей: GRU/LSTM остаётся стабильным baseline/fallback для прогрессий, Transformer используется в первую очередь для коротких мелодий и риффов, а любые Transformer-эксперименты для прогрессий сравниваются с GRU/LSTM перед заменой.
...
- добавить отдельный маленький Transformer для коротких мелодических фраз и риффов после полезного GRU/LSTM baseline для прогрессий;
